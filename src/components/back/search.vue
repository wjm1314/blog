<template>
  <div class="container">
    ...
    <div class="searchString">
      搜索匹配:
      <label for="title">
        <input type="radio" value="title" id="title" v-model="picked">标题
      </label>
      <label for="tags">
        <input type="radio" value="tags" id="tags" v-model="picked">标签
      </label>
      <label for="date">
        <input type="radio" value="date" id="date" v-model="picked">日期
      </label>
    </div>
    <p>搜索结果</p>
  ...
  export default {
    ...
    computed: {
      tip() {
        ...
        if(this.picked === 'tags') {
          return '请输入完整的标签，多个标签空格隔开'
        }
        if(this.picked === 'date') {
          return '检索格式：2018-12-20'
        }
      }
    },
    methods: {
