  ...
  <div class="wrapper" id="article">
    <div id="content">
      ...
      <div class="appendInfo animated fadeIn">
        ...
        <span><i class="iconfont icon-label"></i>{{article.tags | toTag}}</span>
        <a href="#comment" class="commentCount"><i class="iconfont icon-huifu"></i>{{article.comment_n}}</a>
      </div>
      ...
      <div class="indexes animated fadeIn">
        <div class="last animated fadeIn">
          ...
          <router-link :to="{name:'article',params: {id: articles[nextPage].aid,index:nextPage,page:$route.params.page},hash: '#article'}" v-if="articles[nextPage]" tags="p" class="right">
            <i class="iconfont icon-right"></i>{{articles[nextPage].title}}</router-link>
        </div>
      </div>
...
<script>
  ...
  import ArticleComment from './component/ArticleComment.vue'
  ...
  export default {
    ...
    computed: {
      ...mapState(['article','articles','curTag']),
      mdHtml() {
